...
Parameters: $\gamma=1/5$, $\mu=0.01$, $b=-1$, $\sigma=0.5$
...
## Population size ($t=10^5$)

```{r get_res3,cache=TRUE}
rL <- setNames(sizeList,sprintf("N=%d",sizevec))
dd3 <- collect_res(rL) %>%
    filter(var=="mean_lbeta")
rL2 <- setNames(csizeList,sprintf("N=%d",sizevec))
dd3C <- collect_res(rL2) %>%
    filter(var=="mean_lbeta")
```
...
dd4C <- dd3C %>% filter(time>50000) %>%
    mutate(N=as.numeric(gsub("N=","",type)))
dd4comb <- bind_rows(disc=dd4,cont=dd4C,.id="time_type")
gg4 <- ggplot(dd4,
...
```{r disccont}
gg5 <- ggplot(dd4comb,
              aes(x=N,y=val,group=interaction(type,time_type),
                  shape=time_type,
                  colour=as.numeric(factor(type,levels=names(rL)))))+
    stat_summary(fun.data=mean_cl_normal)+
    scale_color_viridis(name="",breaks=seq_along(names(rL)),
...
(e.g. human $N_e$ is $<10,000$)
- boom-and-bust/seasonal fluctuations in SIRS models
(e.g. critical community size)
